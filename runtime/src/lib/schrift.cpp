...
static struct point midpoint(struct point a, struct point b) { return (struct point){0.5 * a.x + 0.5 * b.x, 0.5 * a.y + 0.5 * b.y}; }
...
static void transform_points(int numPts, struct point* points, double trf[6]) {
    ...
    for (i = 0; i < numPts; ++i) {
        pt = &points[i];
        *pt = (struct point){pt->x * trf[0] + pt->y * trf[2] + trf[4], pt->x * trf[1] + pt->y * trf[3] + trf[5]};
    }
}
...
static void free_buffer(struct buffer* buf) { free(buf->rows); }
...
static inline int8_t geti8(SFT_Font* font, unsigned long offset) { return (int8_t)getu8(font, offset); }
...
static inline int16_t geti16(SFT_Font* font, unsigned long offset) { return (int16_t)getu16(font, offset); }
...
static long cmap_fmt4(SFT_Font* font, unsigned long table, unsigned long charCode) {
    ...
    segIdxX2 = (uintptr_t)csearch(key, font->memory + endCodes, segCountX2 / 2, 2, cmpu16) - (uintptr_t)(font->memory + endCodes);
...
static int render_image(const struct SFT* sft, unsigned long offset, double transform[6], struct SFT_Char* chr) {
    ...
    if (!err)
        post_process(buf, (uint8_t*)chr->image);
